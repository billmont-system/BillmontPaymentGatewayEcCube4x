...
namespace Plugin\BillmontPaymentGateway\Controller;
...
class PaymentController extends AbstractController {
  ...
  /**
   * @Route("/billmont_payment/{order_no}/{pre_order_id}/bridge", name="billmont_payment_bridge")
   *
  ...
  public function bridge(Request $request) {
    ...
    $cancelUrl = $baseUrl."billmont_payment".'/'.$orderNo.'/'.$preOrderId.'/back';
    $callbackUrl = $baseUrl."billmont_payment".'/'.$orderNo.'/'.$preOrderId.'/complete';
    ...
    $state = $Order->getPref()->getName();
    $city = htmlspecialchars($Order->getAddr01());
    $address_line1 = htmlspecialchars($Order->getAddr02());

    $html = <<<HTML
<!DOCTYPE html>
<html>
<head>
  <title>Billmont Payment</title>
  <meta charset="utf-8">
  <meta http-equiv="Cache-Control" content="no-cache">
</head>
<body>
  <div style="text-align: center;">
    <img src="https://payment-hub.billmont.net/img/color-logo.png">
    <p style="color: #999;">
      自動でBillmontクレジットカード決済ページに移動します。<br>
      移動しない場合は「支払ページへ移動」をタップしてください。
    </p>
    <form method="POST" action="https://{$api_domain}/redirect/{$api_token}/payment" id="form">
...
HTML;
  ...
  /**
   * @Route("/billmont_payment/{order_no}/{pre_order_id}/back", name="billmont_payment_back")
   *
   * @param Request $request
   *
   * @return RedirectResponse
  ...
  public function back(Request $request) {
    ...
    $Order->setBillmontPaymentStatus($PaymentStatus);
   ...
   * 完了画面へ遷移する.
   *
   * @Route("/billmont_payment/{order_no}/{pre_order_id}/complete", name="billmont_payment_complete")
   */
   ...
   * 結果通知URLを受け取る.
   *
   * @Route("/billmont_payment/receive_complete", name="billmont_payment_receive_complete")
   */
  public function receiveComplete(Request $request) {
    ...
    // 決済承認番号を設定
    $Order->setBillmontPaymentTranCode($tran_code);

    // 明細書名を設定
    $Order->setBillmontPaymentDba($dba);
  ...
  private function getOrderByNo($orderNo, $ignore_status = false) {
    ...
    if($ignore_status === false){
      ...
      $condition['BillmontPaymentStatus'] = $outstandingPaymentStatus;
    }
  ...
  private function getOrderByPreOrderId($preOrderId) {
    ...
    $Order = $this->orderRepository->findOneBy([
      ...
      'BillmontPaymentStatus' => $outstandingPaymentStatus,
    ]);
